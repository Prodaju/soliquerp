...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Debug: Print the worksheet score
    echo "Worksheet Score: " . $_SESSION['worksheet_score'] . "<br>";
    ...
    mysqli_stmt_bind_result($stmt, $current_score);
    mysqli_stmt_fetch($stmt);
    mysqli_stmt_close($stmt);

    // Add worksheet score to the current score
    $new_score = $current_score + $_SESSION['worksheet_score'];

    // Update database with the new score
    $sql = "UPDATE users SET score = ? WHERE username = ?";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "is", $new_score, $username);
    ...
    header("Location: worksheet1.php");
    exit;
}
?>
...
<body>
    <div class="container mt-4">
        ...
        <!--------------------------------------------------------------->
       <head>
        <link rel="stylesheet" href="styles-worksheet1.css">
</head>

<body>
    <div id="container">
        <h1>Computer Parts Workbook</h1>
    ...
    <script src="script-worksheet1.js"></script>
</body>
        <!--------------------------------------------------------------->
